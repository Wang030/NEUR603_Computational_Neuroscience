...
%           qstop      path length at which 'findpaths' actually stopped
...
if (savepths==1) allpths = pths; end;
if (savepths~=1) allpths = []; end;
...
for q=2:qmax
    ...
    if (N<50) len_npths = N*q*10; end;
    if (N>=50) len_npths = 10^(q+1); end;
